...
public class MathanLatexMojo extends AbstractMojo {
  ...
  private void executeSteps(List<Step> stepsToExecute, File source) throws MojoExecutionException {
    ...
    resolveDependencies(workingDirectory);
  ...
  private void provideArtifact(File workingDirectory, String pureName) throws MojoExecutionException {
  ...
  private File createWorkingDirectory() throws MojoExecutionException {
    File workingDirectory = new File(project.getBasedir(), "target/latex/");
  ...
  private List<Dependency> resolveDependencies(File workingDirectory) throws MojoExecutionException {
  ...
  private void copySources(File source, File workingDirectory) throws MojoExecutionException {
    try {
  ...
  private File resolveMainFile(File source, File workingDirectory) throws MojoExecutionException {
    File mainFile;
  ...
  /**
   * Checks, if the given executables can be executed by finding the executables either in the configured {@link #texBin bin directory} or on PATH.
   *
  ...
  private void checkExecutables(List<Step> listExecutables) throws MojoExecutionException {
    List<Step> stepsToFail = listExecutables.stream().filter(step -> Utils.getExecutable(texBin, step.getOperatingSystemName()) == null).collect(Collectors.toList());
  ...
  private void executeStep(Step executionStep, File workingDirectory, File texFile) throws MojoExecutionException {
    File exec = Utils.getExecutable(texBin, executionStep.getOperatingSystemName());
    ...
    try {
      ...
      exitValue = new ProcessExecutor().command(command).directory(workingDirectory).redirectOutput(LatexPluginLogOutputStream.toMavenDebug(getLog(), prefix))
          .redirectError(LatexPluginLogOutputStream.toMavenError(getLog(), prefix)).destroyOnExit().execute().getExitValue();
    } catch (Exception e) {
